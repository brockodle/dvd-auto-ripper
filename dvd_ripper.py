...
def find_dvd_device() -> Path:
    """Find and validate DVD device"""
    devices = [Path('/dev/sr0'), Path('/dev/sr1'), Path('/dev/dvd')]
    
    for device in devices:
        if device.exists():
            try:
                # Test device is readable
                subprocess.run(
                    ['dd', f'if={device}', 'of=/dev/null', 'count=1', 'bs=2048'],
                    capture_output=True,
                    check=True,
                    timeout=5
                )
...
def verify_device_access(device: Path) -> bool:
    """Verify device is accessible and contains a DVD"""
    try:
        ...
        if not os.access(device, os.R_OK):
            logger.error(f"No read permission for {device}")
            return False
        
        # Try basic read test with dd
        ...
        if result.returncode != 0:
            logger.error(f"DD read test failed: {result.stderr}")
            return False
...
def ensure_device_ready(device: Path, max_retries: int = 3) -> bool:
    ...
    for attempt in range(max_retries):
        if verify_device_access(device):
            # Try scan with and without dvdnav
        ...
        try:
            # Unmount if mounted
            subprocess.run(['umount', str(device)], capture_output=True)
            time.sleep(1)
            
            # Eject and close
            subprocess.run(['eject', str(device)], capture_output=True)
            time.sleep(2)
            subprocess.run(['eject', '-t', str(device)], capture_output=True)
            time.sleep(3)
            
            # Try mounting
            subprocess.run(['mount', str(device)], capture_output=True)
            time.sleep(2)
            
            # Verify access after mount
            if verify_device_access(device):
                return True
...
def analyze_duration_patterns(titles: dict) -> dict:
    """Analyze title durations to detect episode patterns"""
    durations = []
    
    # Collect all durations (both titles and chapters)
    for num, info in titles.items():
        duration_min = info['duration'] / 60
        if duration_min < 90:  # Skip compilation-length titles
            durations.append(duration_min)
        
        # Include chapter durations
        for _, chapter_duration in info['chapters']:
            chapter_min = chapter_duration / 60
            if chapter_min >= 1:  # Skip very short chapters
                durations.append(chapter_min)
    
    if not durations:
        return DEFAULT_EPISODE_RANGES
    
    # Sort durations and find clusters
    durations.sort()
    clusters = []
    current_cluster = [durations[0]]
    
    # Group durations that are within 5% of each other
    for dur in durations[1:]:
        if dur - current_cluster[-1] <= current_cluster[0] * 0.05:
            current_cluster.append(dur)
        else:
            if len(current_cluster) >= 2:  # Only keep clusters with multiple entries
    ...
    for i, cluster in enumerate(clusters):
        ...
        ranges[f'cluster_{i}'] = (
            (min_dur * 60) - 10,  # 10 seconds buffer on either end
            (max_dur * 60) + 10
        )
    
    # Log detected ranges
    logger.info("\nDetected episode ranges:")
    for name, (min_sec, max_sec) in ranges.items():
        matches = len([d for d in durations if min_sec/60 <= d <= max_sec/60])
        logger.info(f"{name}: {min_sec/60:.1f}-{max_sec/60:.1f} minutes ({matches} matches)")
    
    return ranges

def get_episode_ranges(titles: dict) -> dict:
    """Get episode duration ranges from detected patterns or user input"""
    # First analyze patterns in the titles
    detected_ranges = analyze_duration_patterns(titles)
    
    rprint("\n[yellow]Would you like to:[/yellow]")
    rprint("1. Use detected ranges")
    rprint("2. Use default ranges")
    rprint("3. Enter custom ranges")
    ...
    if choice == '1':
        ...
        # Custom ranges input (previous implementation)
        ranges = {}
        while True:
            name = Prompt.ask("\nEnter range name (or press Enter to finish)")
            if not name:
                break
                
            try:
                min_min = float(Prompt.ask("  Min duration (minutes)"))
                max_min = float(Prompt.ask("  Max duration (minutes)"))
                
                if min_min > 0 and max_min >= min_min:
                    ranges[name] = (min_min * 60, max_min * 60)
                    rprint(f"[green]Added range: {min_min:.1f}-{max_min:.1f} minutes[/green]")
                else:
                    rprint("[red]Invalid range, skipping[/red]")
            except ValueError:
                rprint("[red]Invalid input, skipping[/red]")
        
        if not ranges:
            rprint("[yellow]No valid ranges entered, using defaults[/yellow]")
            return DEFAULT_EPISODE_RANGES
...
def process_tv_show(output_dir: Path) -> bool:
    ...
    # Get episode ranges based on content analysis
    episode_ranges = analyze_duration_patterns(titles)
...
def cleanup_previous_session():
    ...
    try:
        ...
        lock_file = Path("/var/lock/dvd_ripper.lock")
        if lock_file.exists():
            try:
